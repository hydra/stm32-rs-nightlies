...
///Field `DBLOCKSIZE` reader - Data block size. Define the data block length when the block data transfer mode is selected, block length = 2^(DBLOCKSIZE) bytes
pub type DBLOCKSIZE_R = crate::FieldReader<u8, u8>;
///Field `DBLOCKSIZE` writer - Data block size. Define the data block length when the block data transfer mode is selected, block length = 2^(DBLOCKSIZE) bytes
...
impl R {
    ...
    ///Bit 2 - Data transfer mode selection. This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0).
    #[inline(always)]
    ...
    ///Bits 4:7 - Data block size. Define the data block length when the block data transfer mode is selected, block length = 2^(DBLOCKSIZE) bytes
    #[inline(always)]
