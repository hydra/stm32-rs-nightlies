///Register `MPCBB2_VCTR63` reader
pub struct R(crate::R<MPCBB2_VCTR63_SPEC>);
impl core::ops::Deref for R {
    type Target = crate::R<MPCBB2_VCTR63_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl From<crate::R<MPCBB2_VCTR63_SPEC>> for R {
    #[inline(always)]
    fn from(reader: crate::R<MPCBB2_VCTR63_SPEC>) -> Self {
        R(reader)
    }
}
///Register `MPCBB2_VCTR63` writer
pub struct W(crate::W<MPCBB2_VCTR63_SPEC>);
impl core::ops::Deref for W {
    type Target = crate::W<MPCBB2_VCTR63_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::ops::DerefMut for W {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl From<crate::W<MPCBB2_VCTR63_SPEC>> for W {
    #[inline(always)]
    fn from(writer: crate::W<MPCBB2_VCTR63_SPEC>) -> Self {
        W(writer)
    }
}
///Field `B2016` reader - B2016
pub type B2016_R = crate::BitReader<bool>;
///Field `B2016` writer - B2016
pub type B2016_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2017` reader - B2017
pub type B2017_R = crate::BitReader<bool>;
///Field `B2017` writer - B2017
pub type B2017_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2018` reader - B2018
pub type B2018_R = crate::BitReader<bool>;
///Field `B2018` writer - B2018
pub type B2018_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2019` reader - B2019
pub type B2019_R = crate::BitReader<bool>;
///Field `B2019` writer - B2019
pub type B2019_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2020` reader - B2020
pub type B2020_R = crate::BitReader<bool>;
///Field `B2020` writer - B2020
pub type B2020_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2021` reader - B2021
pub type B2021_R = crate::BitReader<bool>;
///Field `B2021` writer - B2021
pub type B2021_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2022` reader - B2022
pub type B2022_R = crate::BitReader<bool>;
///Field `B2022` writer - B2022
pub type B2022_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2023` reader - B2023
pub type B2023_R = crate::BitReader<bool>;
///Field `B2023` writer - B2023
pub type B2023_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2024` reader - B2024
pub type B2024_R = crate::BitReader<bool>;
///Field `B2024` writer - B2024
pub type B2024_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2025` reader - B2025
pub type B2025_R = crate::BitReader<bool>;
///Field `B2025` writer - B2025
pub type B2025_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2026` reader - B2026
pub type B2026_R = crate::BitReader<bool>;
///Field `B2026` writer - B2026
pub type B2026_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2027` reader - B2027
pub type B2027_R = crate::BitReader<bool>;
///Field `B2027` writer - B2027
pub type B2027_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2028` reader - B2028
pub type B2028_R = crate::BitReader<bool>;
///Field `B2028` writer - B2028
pub type B2028_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2029` reader - B2029
pub type B2029_R = crate::BitReader<bool>;
///Field `B2029` writer - B2029
pub type B2029_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2030` reader - B2030
pub type B2030_R = crate::BitReader<bool>;
///Field `B2030` writer - B2030
pub type B2030_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2031` reader - B2031
pub type B2031_R = crate::BitReader<bool>;
///Field `B2031` writer - B2031
pub type B2031_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2032` reader - B2032
pub type B2032_R = crate::BitReader<bool>;
///Field `B2032` writer - B2032
pub type B2032_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2033` reader - B2033
pub type B2033_R = crate::BitReader<bool>;
///Field `B2033` writer - B2033
pub type B2033_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2034` reader - B2034
pub type B2034_R = crate::BitReader<bool>;
///Field `B2034` writer - B2034
pub type B2034_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2035` reader - B2035
pub type B2035_R = crate::BitReader<bool>;
///Field `B2035` writer - B2035
pub type B2035_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2036` reader - B2036
pub type B2036_R = crate::BitReader<bool>;
///Field `B2036` writer - B2036
pub type B2036_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2037` reader - B2037
pub type B2037_R = crate::BitReader<bool>;
///Field `B2037` writer - B2037
pub type B2037_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2038` reader - B2038
pub type B2038_R = crate::BitReader<bool>;
///Field `B2038` writer - B2038
pub type B2038_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2039` reader - B2039
pub type B2039_R = crate::BitReader<bool>;
///Field `B2039` writer - B2039
pub type B2039_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2040` reader - B2040
pub type B2040_R = crate::BitReader<bool>;
///Field `B2040` writer - B2040
pub type B2040_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2041` reader - B2041
pub type B2041_R = crate::BitReader<bool>;
///Field `B2041` writer - B2041
pub type B2041_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2042` reader - B2042
pub type B2042_R = crate::BitReader<bool>;
///Field `B2042` writer - B2042
pub type B2042_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2043` reader - B2043
pub type B2043_R = crate::BitReader<bool>;
///Field `B2043` writer - B2043
pub type B2043_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2044` reader - B2044
pub type B2044_R = crate::BitReader<bool>;
///Field `B2044` writer - B2044
pub type B2044_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2045` reader - B2045
pub type B2045_R = crate::BitReader<bool>;
///Field `B2045` writer - B2045
pub type B2045_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2046` reader - B2046
pub type B2046_R = crate::BitReader<bool>;
///Field `B2046` writer - B2046
pub type B2046_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
///Field `B2047` reader - B2047
pub type B2047_R = crate::BitReader<bool>;
///Field `B2047` writer - B2047
pub type B2047_W<'a, const O: u8> = crate::BitWriter<'a, u32, MPCBB2_VCTR63_SPEC, bool, O>;
impl R {
    ///Bit 0 - B2016
    #[inline(always)]
    pub fn b2016(&self) -> B2016_R {
        B2016_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - B2017
    #[inline(always)]
    pub fn b2017(&self) -> B2017_R {
        B2017_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - B2018
    #[inline(always)]
    pub fn b2018(&self) -> B2018_R {
        B2018_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - B2019
    #[inline(always)]
    pub fn b2019(&self) -> B2019_R {
        B2019_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4 - B2020
    #[inline(always)]
    pub fn b2020(&self) -> B2020_R {
        B2020_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - B2021
    #[inline(always)]
    pub fn b2021(&self) -> B2021_R {
        B2021_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6 - B2022
    #[inline(always)]
    pub fn b2022(&self) -> B2022_R {
        B2022_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7 - B2023
    #[inline(always)]
    pub fn b2023(&self) -> B2023_R {
        B2023_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - B2024
    #[inline(always)]
    pub fn b2024(&self) -> B2024_R {
        B2024_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - B2025
    #[inline(always)]
    pub fn b2025(&self) -> B2025_R {
        B2025_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - B2026
    #[inline(always)]
    pub fn b2026(&self) -> B2026_R {
        B2026_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 11 - B2027
    #[inline(always)]
    pub fn b2027(&self) -> B2027_R {
        B2027_R::new(((self.bits >> 11) & 1) != 0)
    }
    ///Bit 12 - B2028
    #[inline(always)]
    pub fn b2028(&self) -> B2028_R {
        B2028_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13 - B2029
    #[inline(always)]
    pub fn b2029(&self) -> B2029_R {
        B2029_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - B2030
    #[inline(always)]
    pub fn b2030(&self) -> B2030_R {
        B2030_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15 - B2031
    #[inline(always)]
    pub fn b2031(&self) -> B2031_R {
        B2031_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 16 - B2032
    #[inline(always)]
    pub fn b2032(&self) -> B2032_R {
        B2032_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - B2033
    #[inline(always)]
    pub fn b2033(&self) -> B2033_R {
        B2033_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18 - B2034
    #[inline(always)]
    pub fn b2034(&self) -> B2034_R {
        B2034_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bit 19 - B2035
    #[inline(always)]
    pub fn b2035(&self) -> B2035_R {
        B2035_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20 - B2036
    #[inline(always)]
    pub fn b2036(&self) -> B2036_R {
        B2036_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21 - B2037
    #[inline(always)]
    pub fn b2037(&self) -> B2037_R {
        B2037_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22 - B2038
    #[inline(always)]
    pub fn b2038(&self) -> B2038_R {
        B2038_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23 - B2039
    #[inline(always)]
    pub fn b2039(&self) -> B2039_R {
        B2039_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bit 24 - B2040
    #[inline(always)]
    pub fn b2040(&self) -> B2040_R {
        B2040_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bit 25 - B2041
    #[inline(always)]
    pub fn b2041(&self) -> B2041_R {
        B2041_R::new(((self.bits >> 25) & 1) != 0)
    }
    ///Bit 26 - B2042
    #[inline(always)]
    pub fn b2042(&self) -> B2042_R {
        B2042_R::new(((self.bits >> 26) & 1) != 0)
    }
    ///Bit 27 - B2043
    #[inline(always)]
    pub fn b2043(&self) -> B2043_R {
        B2043_R::new(((self.bits >> 27) & 1) != 0)
    }
    ///Bit 28 - B2044
    #[inline(always)]
    pub fn b2044(&self) -> B2044_R {
        B2044_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bit 29 - B2045
    #[inline(always)]
    pub fn b2045(&self) -> B2045_R {
        B2045_R::new(((self.bits >> 29) & 1) != 0)
    }
    ///Bit 30 - B2046
    #[inline(always)]
    pub fn b2046(&self) -> B2046_R {
        B2046_R::new(((self.bits >> 30) & 1) != 0)
    }
    ///Bit 31 - B2047
    #[inline(always)]
    pub fn b2047(&self) -> B2047_R {
        B2047_R::new(((self.bits >> 31) & 1) != 0)
    }
}
impl W {
    ///Bit 0 - B2016
    #[inline(always)]
    #[must_use]
    pub fn b2016(&mut self) -> B2016_W<0> {
        B2016_W::new(self)
    }
    ///Bit 1 - B2017
    #[inline(always)]
    #[must_use]
    pub fn b2017(&mut self) -> B2017_W<1> {
        B2017_W::new(self)
    }
    ///Bit 2 - B2018
    #[inline(always)]
    #[must_use]
    pub fn b2018(&mut self) -> B2018_W<2> {
        B2018_W::new(self)
    }
    ///Bit 3 - B2019
    #[inline(always)]
    #[must_use]
    pub fn b2019(&mut self) -> B2019_W<3> {
        B2019_W::new(self)
    }
    ///Bit 4 - B2020
    #[inline(always)]
    #[must_use]
    pub fn b2020(&mut self) -> B2020_W<4> {
        B2020_W::new(self)
    }
    ///Bit 5 - B2021
    #[inline(always)]
    #[must_use]
    pub fn b2021(&mut self) -> B2021_W<5> {
        B2021_W::new(self)
    }
    ///Bit 6 - B2022
    #[inline(always)]
    #[must_use]
    pub fn b2022(&mut self) -> B2022_W<6> {
        B2022_W::new(self)
    }
    ///Bit 7 - B2023
    #[inline(always)]
    #[must_use]
    pub fn b2023(&mut self) -> B2023_W<7> {
        B2023_W::new(self)
    }
    ///Bit 8 - B2024
    #[inline(always)]
    #[must_use]
    pub fn b2024(&mut self) -> B2024_W<8> {
        B2024_W::new(self)
    }
    ///Bit 9 - B2025
    #[inline(always)]
    #[must_use]
    pub fn b2025(&mut self) -> B2025_W<9> {
        B2025_W::new(self)
    }
    ///Bit 10 - B2026
    #[inline(always)]
    #[must_use]
    pub fn b2026(&mut self) -> B2026_W<10> {
        B2026_W::new(self)
    }
    ///Bit 11 - B2027
    #[inline(always)]
    #[must_use]
    pub fn b2027(&mut self) -> B2027_W<11> {
        B2027_W::new(self)
    }
    ///Bit 12 - B2028
    #[inline(always)]
    #[must_use]
    pub fn b2028(&mut self) -> B2028_W<12> {
        B2028_W::new(self)
    }
    ///Bit 13 - B2029
    #[inline(always)]
    #[must_use]
    pub fn b2029(&mut self) -> B2029_W<13> {
        B2029_W::new(self)
    }
    ///Bit 14 - B2030
    #[inline(always)]
    #[must_use]
    pub fn b2030(&mut self) -> B2030_W<14> {
        B2030_W::new(self)
    }
    ///Bit 15 - B2031
    #[inline(always)]
    #[must_use]
    pub fn b2031(&mut self) -> B2031_W<15> {
        B2031_W::new(self)
    }
    ///Bit 16 - B2032
    #[inline(always)]
    #[must_use]
    pub fn b2032(&mut self) -> B2032_W<16> {
        B2032_W::new(self)
    }
    ///Bit 17 - B2033
    #[inline(always)]
    #[must_use]
    pub fn b2033(&mut self) -> B2033_W<17> {
        B2033_W::new(self)
    }
    ///Bit 18 - B2034
    #[inline(always)]
    #[must_use]
    pub fn b2034(&mut self) -> B2034_W<18> {
        B2034_W::new(self)
    }
    ///Bit 19 - B2035
    #[inline(always)]
    #[must_use]
    pub fn b2035(&mut self) -> B2035_W<19> {
        B2035_W::new(self)
    }
    ///Bit 20 - B2036
    #[inline(always)]
    #[must_use]
    pub fn b2036(&mut self) -> B2036_W<20> {
        B2036_W::new(self)
    }
    ///Bit 21 - B2037
    #[inline(always)]
    #[must_use]
    pub fn b2037(&mut self) -> B2037_W<21> {
        B2037_W::new(self)
    }
    ///Bit 22 - B2038
    #[inline(always)]
    #[must_use]
    pub fn b2038(&mut self) -> B2038_W<22> {
        B2038_W::new(self)
    }
    ///Bit 23 - B2039
    #[inline(always)]
    #[must_use]
    pub fn b2039(&mut self) -> B2039_W<23> {
        B2039_W::new(self)
    }
    ///Bit 24 - B2040
    #[inline(always)]
    #[must_use]
    pub fn b2040(&mut self) -> B2040_W<24> {
        B2040_W::new(self)
    }
    ///Bit 25 - B2041
    #[inline(always)]
    #[must_use]
    pub fn b2041(&mut self) -> B2041_W<25> {
        B2041_W::new(self)
    }
    ///Bit 26 - B2042
    #[inline(always)]
    #[must_use]
    pub fn b2042(&mut self) -> B2042_W<26> {
        B2042_W::new(self)
    }
    ///Bit 27 - B2043
    #[inline(always)]
    #[must_use]
    pub fn b2043(&mut self) -> B2043_W<27> {
        B2043_W::new(self)
    }
    ///Bit 28 - B2044
    #[inline(always)]
    #[must_use]
    pub fn b2044(&mut self) -> B2044_W<28> {
        B2044_W::new(self)
    }
    ///Bit 29 - B2045
    #[inline(always)]
    #[must_use]
    pub fn b2045(&mut self) -> B2045_W<29> {
        B2045_W::new(self)
    }
    ///Bit 30 - B2046
    #[inline(always)]
    #[must_use]
    pub fn b2046(&mut self) -> B2046_W<30> {
        B2046_W::new(self)
    }
    ///Bit 31 - B2047
    #[inline(always)]
    #[must_use]
    pub fn b2047(&mut self) -> B2047_W<31> {
        B2047_W::new(self)
    }
    ///Writes raw bits to the register.
    #[inline(always)]
    pub unsafe fn bits(&mut self, bits: u32) -> &mut Self {
        self.0.bits(bits);
        self
    }
}
///MPCBBx vector register
///
///This register you can [`read`](crate::generic::Reg::read), [`write_with_zero`](crate::generic::Reg::write_with_zero), [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`modify`](crate::generic::Reg::modify). See [API](https://docs.rs/svd2rust/#read--modify--write-api).
///
///For information about available fields see [mpcbb2_vctr63](index.html) module
pub struct MPCBB2_VCTR63_SPEC;
impl crate::RegisterSpec for MPCBB2_VCTR63_SPEC {
    type Ux = u32;
}
///`read()` method returns [mpcbb2_vctr63::R](R) reader structure
impl crate::Readable for MPCBB2_VCTR63_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mpcbb2_vctr63::W](W) writer structure
impl crate::Writable for MPCBB2_VCTR63_SPEC {
    type Writer = W;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: Self::Ux = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: Self::Ux = 0;
}
///`reset()` method sets MPCBB2_VCTR63 to value 0
impl crate::Resettable for MPCBB2_VCTR63_SPEC {
    const RESET_VALUE: Self::Ux = 0;
}
