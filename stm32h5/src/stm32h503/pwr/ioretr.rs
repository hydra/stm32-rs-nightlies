...
///Field `IORETEN` reader - IO retention enable: When entering into standby mode, the output is sampled, and applied to the output IO during the standby power mode. Note: the IO state is not retained if the DBG_STANDBY bit is set in DBGMCU_CR register.
pub type IORETEN_R = crate::BitReader<bool>;
///Field `IORETEN` writer - IO retention enable: When entering into standby mode, the output is sampled, and applied to the output IO during the standby power mode. Note: the IO state is not retained if the DBG_STANDBY bit is set in DBGMCU_CR register.
pub type IORETEN_W<'a, const O: u8> = crate::BitWriter<'a, u32, IORETR_SPEC, bool, O>;
///Field `JTAGIORETEN` reader - IO retention enable for JTAG IOs when entering into standby mode, the output is sampled, and applied to the output IO during the standby power mode
pub type JTAGIORETEN_R = crate::BitReader<bool>;
///Field `JTAGIORETEN` writer - IO retention enable for JTAG IOs when entering into standby mode, the output is sampled, and applied to the output IO during the standby power mode
