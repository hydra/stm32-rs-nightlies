...
///Field `TIMEOUT` reader - 15: 0\]: Timeout period After each access in Memory-mapped mode, the OCTOSPI prefetches the subsequent bytes and hold them in the FIFO. This field indicates how many CLK cycles the OCTOSPI waits after the clock becomes inactive and until it raises the NCS, putting the external device in a lower-consumption state.
pub type TIMEOUT_R = crate::FieldReader<u16, u16>;
///Field `TIMEOUT` writer - 15: 0\]: Timeout period After each access in Memory-mapped mode, the OCTOSPI prefetches the subsequent bytes and hold them in the FIFO. This field indicates how many CLK cycles the OCTOSPI waits after the clock becomes inactive and until it raises the NCS, putting the external device in a lower-consumption state.
pub type TIMEOUT_W<'a, const O: u8> = crate::FieldWriter<'a, u32, LPTR_SPEC, u16, u16, 16, O>;
impl R {
    ///Bits 0:15 - 15: 0\]: Timeout period After each access in Memory-mapped mode, the OCTOSPI prefetches the subsequent bytes and hold them in the FIFO. This field indicates how many CLK cycles the OCTOSPI waits after the clock becomes inactive and until it raises the NCS, putting the external device in a lower-consumption state.
    #[inline(always)]
