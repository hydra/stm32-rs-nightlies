...
///Field `BOR_LEV` reader - Brownout level option configuration bit These bits reflects the power level that generates a system reset. 00 or 11: BOR Level 1, the threshold level is low (around 2.1 V)
pub type BOR_LEV_R = crate::FieldReader<u8, u8>;
///Field `BOR_LEV` writer - Brownout level option configuration bit These bits reflects the power level that generates a system reset. 00 or 11: BOR Level 1, the threshold level is low (around 2.1 V)
...
///Field `PRODUCT_STATE` writer - Life state code (based on Hamming 8,4). More information in Section�7.6.11: Product state transitions.
pub type PRODUCT_STATE_W<'a, const O: u8> =
    crate::FieldWriter<'a, u32, OPTSR_PRG_SPEC, u8, u8, 8, O>;
///Field `IO_VDD_HSLV` reader - High-speed IO at low VDD voltage configuration bit. This bit can be set only with VDD below 2.7�V.
pub type IO_VDD_HSLV_R = crate::BitReader<bool>;
///Field `IO_VDD_HSLV` writer - High-speed IO at low VDD voltage configuration bit. This bit can be set only with VDD below 2.7�V.
pub type IO_VDD_HSLV_W<'a, const O: u8> = crate::BitWriter<'a, u32, OPTSR_PRG_SPEC, bool, O>;
///Field `IO_VDDIO2_HSLV` reader - High-speed IO at low VDDIO2 voltage configuration bit. This bit can be set only with VDDIO2 below 2.7�V.
pub type IO_VDDIO2_HSLV_R = crate::BitReader<bool>;
///Field `IO_VDDIO2_HSLV` writer - High-speed IO at low VDDIO2 voltage configuration bit. This bit can be set only with VDDIO2 below 2.7�V.
...
///Field `IWDG_STOP` writer - IWDG Stop mode freeze option status bit When set the independent watchdog IWDG is in system Stop mode.
...
///Field `IWDG_STDBY` writer - IWDG Standby mode freeze option status bit When set the independent watchdog IWDG is frozen in system Standby mode.
pub type IWDG_STDBY_W<'a, const O: u8> = crate::BitWriter<'a, u32, OPTSR_PRG_SPEC, bool, O>;
///Field `BOOT_UBE` reader - Available only on cryptography enabled devices. Unique boot entry control, selects either ST or OEM iRoT for secure boot. In Open PRODUCT_STATE this value selects bootloader. Defaut value.
pub type BOOT_UBE_R = crate::FieldReader<u8, u8>;
///Field `BOOT_UBE` writer - Available only on cryptography enabled devices. Unique boot entry control, selects either ST or OEM iRoT for secure boot. In Open PRODUCT_STATE this value selects bootloader. Defaut value.
...
impl R {
    ///Bits 0:1 - Brownout level option configuration bit These bits reflects the power level that generates a system reset. 00 or 11: BOR Level 1, the threshold level is low (around 2.1 V)
    #[inline(always)]
    ...
    ///Bit 16 - High-speed IO at low VDD voltage configuration bit. This bit can be set only with VDD below 2.7�V.
    #[inline(always)]
    pub fn io_vdd_hslv(&self) -> IO_VDD_HSLV_R {
        IO_VDD_HSLV_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - High-speed IO at low VDDIO2 voltage configuration bit. This bit can be set only with VDDIO2 below 2.7�V.
    #[inline(always)]
    ...
    ///Bits 22:29 - Available only on cryptography enabled devices. Unique boot entry control, selects either ST or OEM iRoT for secure boot. In Open PRODUCT_STATE this value selects bootloader. Defaut value.
    #[inline(always)]
